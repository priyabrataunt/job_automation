...
// Curated JobRight new-grad / tech lists — filterJob still applies per listing.
const JOBRIGHT_SOURCES = [
  {
  ...
  {
    url: 'https://raw.githubusercontent.com/jobright-ai/2026-Engineering-New-Grad/master/README.md',
    label: 'Engineering New Grad',
  },
  {
    url: 'https://raw.githubusercontent.com/jobright-ai/2026-Data-Analysis-New-Grad/master/README.md',
    label: 'Data Analysis New Grad',
  },
  {
...
async function collectFromSource(
  ...
  const { data } = await axios.get<string>(sourceUrl, { timeout: 20000 });
...
export async function collectJobRight(hoursBack: number): Promise<Job[]> {
  ...
  const results = await Promise.allSettled(
    JOBRIGHT_SOURCES.map(source =>
      collectFromSource(source.url, source.label, cutoff, nowIso)
    )
  );
  ...
  for (const result of results) {
    if (result.status === 'fulfilled') {
      ...
      console.warn(`[JobRight] ${result.reason?.message}`);
    }
  }
