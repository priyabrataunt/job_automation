...
export async function sendDigestPush(): Promise<void> {
    ...
    const stats = await db.prepare(`SELECT COUNT(*)::int as total FROM jobs WHERE first_seen_at >= ?`).get(cutoff) as {
        total: number;
    };
    const topJob = await db.prepare(`SELECT title, company FROM jobs WHERE first_seen_at >= ? ORDER BY relevance_score DESC LIMIT 1`).get(cutoff) as {
        title: string;
        company: string;
    } | undefined;
    if (stats.total === 0)
    ...
    const subscriptions = await db.prepare('SELECT * FROM push_subscriptions').all() as {
        id: number;
        endpoint: string;
        keys_p256dh: string;
        keys_auth: string;
