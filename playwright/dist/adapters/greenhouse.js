...
exports.greenhouse = {
    ...
    async detect(page) {
        try {
            ...
            const domMatch = await page.evaluate(() => {
                if (document.querySelector('[data-source="greenhouse"]'))
                    return true;
                if (document.querySelector('#app_body .application'))
                    return true;
                if (document.title.toLowerCase().includes('greenhouse'))
                    return true;
                return false;
            });
            return domMatch;
    ...
    async handleMultiStep(page) {
        try {
            ...
            const isDisabled = await toClick.evaluate(el => el.disabled || el.getAttribute('aria-disabled') === 'true');
            if (isDisabled)
                return false;
            await toClick.click();
