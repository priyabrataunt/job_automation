...
const dropdown_handler_1 = require("../dropdown-handler");
exports.generic = {
    name: 'generic',
    async detect(_page) {
        return true;
    ...
    async fillForm(page, engine, job) {
        ...
        const rawFieldInfos = await page.evaluate(() => {
            const results = [];
            const candidateSelector = 'input:not([type="hidden"]):not([type="submit"]):not([type="button"]):not([type="reset"]), ' +
            ...
            const normalize = (text) => text.replace(/\s+/g, ' ').trim().replace(/\s*\*+\s*$/, '');
            const extractByIds = (ids) => {
                if (!ids)
                    return '';
                return ids
                    .split(/\s+/)
            ...
            const buildSelector = (element, index) => {
                if (element.id)
                    return `#${element.id}`;
            ...
            const inferLabel = (element) => {
                ...
                if (enclosingLabel?.textContent)
                    return normalize(enclosingLabel.textContent);
            ...
            for (let index = 0; index < elements.length; index++) {
                ...
                const isAutocomplete = ariaAutocomplete === 'list' ||
        ...
        const fieldInfos = rawFieldInfos.map((field) => ({
            ...field,
            type: field.type,
        }));
        ...
        const formFields = uniqueFields.map((field) => ({
            label: field.label,
            type: field.type,
        ...
        const remainingByLabel = new Map();
                ...
                switch (info.type) {
                    ...
                    case 'textarea': {
                        if (info.isAutocomplete) {
                            const selected = await (0, dropdown_handler_1.fillAutocomplete)(page, info.selector, result.value);
                            if (!selected)
                    ...
                    case 'select': {
                        if (info.isCustomDropdown || info.isAutocomplete) {
                            const selected = await (0, dropdown_handler_1.selectCustomDropdown)(page, info.selector, result.value);
                            if (!selected && info.isAutocomplete) {
                                await (0, dropdown_handler_1.fillAutocomplete)(page, info.selector, result.value);
                            }
                            break;
                        }
                        try {
                    ...
                    case 'radio': {
                        ...
                        for (let index = 0; index < count; index++) {
                            ...
                            const labelText = await radio.evaluate((element) => {
    ...
    async handleMultiStep(page) {
        try {
            ...
            if (count === 0)
                return false;
            const tag = await nextButton.evaluate((el) => el.type?.toLowerCase());
            if (tag === 'submit')
                return false;
    ...
    async getSubmitButton(page) {
        ...
        try {
            ...
            if (count > 0)
                return await textButton.elementHandle();
        }
        catch {
            // ignore
        }
